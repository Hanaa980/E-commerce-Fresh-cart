...
export default function Btn({ product }) {
  ...
  async function handleUpdateCountToCart(productId, count) {
    setIsLoader(true);
    let data = await updateCountToCart(productId, count);
    setIsLoader(false);
  }

  const cartProduct = cartDetails?.data?.products?.find(
  ...
  return (
    <>
      <div className="flex justify-between m-3 w-[90%]  ">
        ...
        {isChecked && productCount > 0 ? (
          <div className="flex w-[50px] items-center justify-center ">
            <button
              onClick={() =>
                handleUpdateCountToCart(product.id, productCount - 1)
              }
              className="inline-flex items-center justify-center p-1 me-1 text-sm font-medium h-6 w-6 text-gray-500 bg-white border border-gray-300 rounded-full focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-200 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700"
              type="button"
            >
          ...
          <button
            ...
            className="btn bg-main p-2 rounded-lg text-white"
          >
            {isLoader ? <ClipLoader size={20} color="#ffffff" /> : "Add +"}
          </button>
        )}
      </div>
    </>
